...
def binary_image(srcImage):
  ...
  gray = cv2.cvtColor(srcImage,
                      cv2.COLOR_RGB2GRAY)  # convert 3D channel to 1D
  ...
  _, binaryImage = cv2.threshold(gray, 150, 255, cv2.THRESH_BINARY)
...
def ocr_text_from_image(regions, srcImage, path):
  ...
  for region in regions:
    ...
    Configer.SCALE = 10
    textImage = cv2.resize(regionImage,
                           (int(Configer.SCALE * w), int(Configer.SCALE * h)),
                           interpolation=cv2.INTER_CUBIC)
    if Configer.DEBUG and regionImage.size > 0:
      cv2.imwrite(os.path.join(path, str(idx) + "_resized_image_" + str(
        Configer.SCALE) + ".png"), textImage)

    # textImage = regionImage
    # call tersseract to reconize text
    result = pytesseract.image_to_string(textImage, lang='eng',
                                         config=Configer.TERSSERACT_CONFIG)
    print("recongnize " + str(idx) + " as " + result + "\n")
    if result is not None:  # and result != '':
      text.append(result)
      result_fp.write(
        str(idx).encode('utf-8') + ":".encode('utf-8') + result + '\n'.encode(
          'utf-8'))
    else:
      text.append(u'null\n')
      result_fp.write(
        str(idx).encode('utf-8') + ": null".encode('utf-8') + '\n'.encode(
          'utf-8'))
...
def find_lines_inregion(img, arrow_region, resimg=None,
                        minexp=1e-8):  # img = ori_img_binary.copy()
  ...
  if contours:
    ...
    if x:
      ...
      if resimg:
        if vx < minexp:
          cv2.line(resimg, (x_img, resimg.shape[0] - 1), (x_img, 0),
                   (255, 255, 0), 2)
        else:
          ...
          cv2.line(resimg, (resimg.shape[1] - 1, righty), (0, lefty),
                   (255, 255, 0), 2)
...
def get_candidate_seeds(region, size_w, size_h, scale_out=1.5, scale_in=0.4):
  ...
  return seeds, [outer_left_x, outer_up_y, outer_right_x, outer_down_y], [
    in_left_x, in_up_y, in_right_x, in_down_y]
...
def detect_connect_regions(seed1, seed2, img):  # img = ori_img_binary
  ...
  for seedPoint in seed1:
    ...
    if img[(seedPoint[1], seedPoint[0])] == 0:  # the seed point must be black
      ...
      mask[:] = 0
      _ = cv2.floodFill(im_floodfill, mask, seed_pt, 100, (0,), (0,),
                        flags=8 | cv2.FLOODFILL_FIXED_RANGE)
      cv2.imwrite('floodfill_test.png', im_floodfill)

      for s2 in seed2:
        if im_floodfill[(
        s2[1], s2[0])] == 100:  # the filled region has overlap with target box
          print("find seed2!")
          return 1
...
def get_predicted_regions(input_image, input_predict):
  ...
  for line in results:
    ...
    left_y = int(np.floor(min([geo[1], geo[3], geo[5],
                               geo[7]])))  # int(geo[geo.index(str(left_x))+1])
    right_y = int(np.ceil(max([geo[1], geo[3], geo[5],
                               geo[7]])))  # int(geo[geo.index(str(right_x))+1])
  ...
  cv2.imwrite(input_image[:-4]+'_marked_image.png', resImage)
  del ori_img, resImage
...
def plot_connections(img, file, connect_regions):
  for c in connect_regions:
    ...
    rect1center = (int((region1[0] + region1[2]) / 2),int((region1[1] +
                                                            region1[3]) / 2))
    rect2center = (int((region2[0] + region2[2]) / 2), int((region2[1] +
                                                            region2[3]) / 2))
    cv2.line(img, rect1center, rect2center, color=(255, 0, 0), thickness=1)
    cv2.imwrite(file, img)


def extract_interactions_in_figure(figure_file, text_label_file):
  #input_image = "cin_00090.jpg"

  regions, arrow_regions = get_predicted_regions(figure_file, text_label_file)
  ori_img = cv2.imread(figure_file)
  #######test single start##################
  ...
  cv2.imwrite(figure_file[:-4]+'_binary' + figure_file[-4:], ori_img_binary)

  connect_regions = []
  text_centers = [((x[0] + x[2]) / 2, (x[1] + x[3]) / 2) for x in regions]
  resImage2 = ori_img.copy()
  for arrow_region in arrow_regions:
    acenter = ((arrow_region[0] + arrow_region[2]) / 2,
               (arrow_region[1] + arrow_region[3]) / 2)
    ...
    for i in range(len(regions)):
      ...
      if arrow_region[0] >= xl and arrow_region[2] <= xr and arrow_region[
        1] >= yl and arrow_region[3] <= yr:
        print("1 one arrow in correct place")
        if dist(vx, vy, x, y, target_center) <= 10 and dist(vx, vy, x, y,
                                                            from_center) <= 10:
    ...
    if from_assign:
      ...
      from_seed, out_box2, in_box2 = get_candidate_seeds(
          from_assign[nearest_from_index], ori_img_binary.shape[1],
          ori_img_binary.shape[0], scale_out=2)
      ...
      if detect_connect_regions(arrow_seeds, from_seed, ori_img_binary.copy()):
        ...
        connect_regions.append(
            (region_target, from_assign[nearest_from_index]))

  cv2.imwrite('marked_image.png', resImage2)
  plot_connections(ori_img.copy(), "connetion_plot_fromarrow.png",
                   connect_regions)
  del resImage2,from_assign, from_centers


if __name__ == '__main__':
  extract_interactions_in_figure(r'C:\Users\LSC-110\Desktop\test\cin_00090.jpg',
                                   r'C:\Users\LSC-110\Desktop\test\cin_00090.txt')