...
class East:
    ...
    def __init__(self):
        ...
        self.backbone_architecture = [vgg16.get_layer('block%d_pool' % i).output
                  for i in cfg.feature_layers_range]
        self.backbone_architecture.insert(0, None)
        self.diff = cfg.feature_layers_range[0] - cfg.feature_layers_num

    def gradually_merge(self, i):
        ...
        if i == cfg.feature_layers_num:
            bn = BatchNormalization()(self.head_architecture(i))
            return Conv2D(32, 3, activation='relu', padding='same')(bn)
        else:
            return UpSampling2D((2, 2))(self.head_architecture(i))

    def head_architecture(self, i):
        # i+diff in cfg.feature_layers_range
        assert i + self.diff in cfg.feature_layers_range, \
            ('i=%d+diff=%d not in ' % (i, self.diff)) + \
            str(cfg.feature_layers_range)
    ...
    def east_network(self):
        ...
        cls_code = Conv2D(3, 1, padding='same', name='cls_code'
                              )(before_output)
