...
def get_arrow_boundingbox(img, arrow_shapes, text_shapes):
    temp_img = img.copy()
    erase_all_text(temp_img, text_shapes)
    binary_image = cv2.adaptiveThreshold(cv2.cvtColor(temp_img, cv2.COLOR_RGB2GRAY),
                                         255,
    ...
    candidate_contours, hierarchy = cv2.findContours(binary_image_INV, cv2.RETR_CCOMP,
...
def draw_inhibit_in_arrow_box(img, file_name, arrow_box, head_box):
    ...
    if slope is not None:
        ...
        cv2.line(img, (int(fixed_x + coefficient_x),
                 ...
                 (r, g, b), thickness=inhibit_thickness)
        if slope != 0:
            box_width = head_box_length + 10
            box_hight = 20
            angel = atan(- 1 / slope)
            box = cv2.boxPoints(
                ((fixed_x, fixed_y), (box_width, box_hight), angel))
            box = np.int0(box)
...
def simulate_inhibit_perimg(img_fold,image,sim_json_fold):
    ...
    arrow_boxes, head_boxes = get_arrow_boundingbox(img, arrow_shapes,text_shapes)
    
    # sim = generate_inhibit(img, img_file,arrow_boxes, head_boxes)
    sim, bounding_box_list = generate_inhibit(img, img_file, arrow_boxes, head_boxes)
    ...
    for arrow_shape in arrow_shapes:
        for idx in range(0, len(head_boxes)):
            ...
            if arrow_shape['points'] == head_boxes[idx]:
                ...
                arrow_shape['label'] = str(arrow_shape['label']).replace('activate', 'inhibit')
                break
    ...
    #save json
    label.save(os.path.join(sim_file[:-suffixlen] + '.json'),shapes,image,None,None,None,None,{})

#
#if __name__ == "__main__":
#    img_fold = r'../Duolin/done/'
#    sim_json_fold = r'../Duolin/test_image_inhibit/'
#    image = r'_pmc_articles_instance_3845601_bin_cjc-32-07-380-g002.jpg'
#    simulate_inhibit_perimg(img_fold,image,sim_json_fold)
#
