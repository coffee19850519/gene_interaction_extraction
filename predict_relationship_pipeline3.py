...
from loadexcldata import load_dictinary_from_excl
...
top_model_path = 'bottleneck_fc_model11.h5'
...
def get_sub_images():
    ...
    for imagename in os.listdir(pathway_image_folder):
        ...
        if imagename != ".DS_Store":
            ...
            subimage_boxes, subimage_entity_boxes, relationship_shapes = generate_all_sub_image_in_figure_with_ocr(pathway_image_folder,imagename,ocr_sub_image_folder,10)
...
# load in all subimages in the data/test directory 
...
def get_relationship_pairs(all_subimages_boxes,all_subimages_entity_boxes,filenames,predicted_classes,all_relationship_shapes):

    print("\n****Extracting Relationship Pairs****\n")

    # get only filenames of sub_images with relationship
    counter = 0
    ...
    for sub_image in files_with_relationship:
        ...
        # get that pathway's subimage_boxes and subimage_entity_boxes
        for original_imagename in all_subimages_boxes:
            if sub_image in all_subimages_boxes.get(original_imagename).keys():
                subimage_boxes = all_subimages_boxes.get(original_imagename)
                subimage_entity_boxes = all_subimages_entity_boxes.get(original_imagename)
                original_filename = original_imagename
                break

        # get the boundaries for the current subimage
        box_coordinates = subimage_boxes.get(sub_image)
        entity_boxes = subimage_entity_boxes.get(sub_image)
        candidate_shapes = []

        # get all the arrows and nocks for this subimage's original image
        subimage_relationship_shapes = all_relationship_shapes.get(original_filename)
        ...
        for shape in subimage_relationship_shapes:
        ...
        if len(candidate_shapes) > 1:
            for shape in candidate_shapes:
                temp_distance1 = caculate_distance_between_two_boxes(shape['points'], entity_boxes[0])
                temp_distance2 = caculate_distance_between_two_boxes(shape['points'], entity_boxes[1])
                temp_sum = temp_distance1 + temp_distance2
                if smallest_distance == None or temp_sum < smallest_distance:
        ...
        if index != -1:
            temp_imagename = sub_image.split("_")
            starter = temp_imagename[0]
            receptor = temp_imagename[1][:-4]
...
def generate_all_sub_image_in_figure_with_ocr(image_path,img_file,ocr_sub_image_folder,offset):
    # load pathway image
    ...
    # get the predicted bounding box for text, arrows, and nocks
    predict_gene_box,_ = predict(east_detect, os.path.join(image_path, img_file),quiet=True)

    # clear directory for OCR entities
    for filename in os.listdir(ocr_sub_image_folder):
        filepath = os.path.join(ocr_sub_image_folder, filename)
        try:
    ...
    # get OCR results for each detected entity
    dictionary_file = "gene_dictionary.xls"
    user_words = load_dictinary_from_excl(dictionary_file,'Sheet1',0)
    OCR_results, coords = OCR(image_path,img_file,ocr_sub_image_folder,predict_gene_box)
    corrections = map_result_to_dictionary(OCR_results, user_words, 60)

    # get shapes
    relationship_shapes, gene_shapes = get_shapes_from_prediction_box(predict_gene_box, corrections)

    # get original image dimensions
    x_dimension = np.shape(img)[0]
    y_dimension = np.shape(img)[1]
    original_img_dimensions = x_dimension * y_dimension

    # compare all genes to one another with nested for loops
    subimage_box_coordinates = {}
    subimage_entity_boxes = {}
    for start_idx in range(0, len(gene_shapes)):
        for end_idx in range(start_idx + 1, len(gene_shapes)):
            sub_img, sub_boxes,left_top_y,left_top_x,right_bottom_y,right_bottom_x = generate_sub_image_bounding_two_entities(
                img,
                gene_shapes[start_idx]['points'],
                gene_shapes[end_idx]['points']
                , offset)
            ...
            if sub_img is not None:
                ...
                if path.exists(os.path.join("data/test/not_classified", file_name)):
                    for x in range(2, 2000):
                        if path.exists(os.path.join("data/test/not_classified", startor+'_'+receptor+'_'+str(x)+'.png')):
                            continue
                        else:
                ...
                # create file
                cv2.imwrite(os.path.join("data/test/not_classified", file_name),sub_img)
                subimage_box_coordinates[file_name] = (left_top_x,left_top_y,right_bottom_x,right_bottom_y)
                subimage_entity_boxes[file_name] = (gene_shapes[start_idx]['points'],gene_shapes[end_idx]['points'])

            else:
                print('exception startor:'+gene_shapes[start_idx][
                    'label']+'receptor:'+gene_shapes[end_idx]['label'])

    return subimage_box_coordinates, subimage_entity_boxes, relationship_shapes
...
def caculate_distance_between_two_boxes(box1, box2):
...
if __name__== "__main__":
    # get subimages
    all_subimages_boxes, all_subimages_entity_boxes, all_relationship_shapes = get_sub_images()
    # predict subimages' relationships
    filenames, predicted_classes = predict_relationships()
    # extract and output relationship pairs
    get_relationship_pairs(all_subimages_boxes,all_subimages_entity_boxes,filenames,predicted_classes, all_relationship_shapes)