...
def quad_loss(y_true, y_pred):
    ...
    cls_logits = y_pred[:, :, :, 1:4]
    cls_labels = y_true[:, :, :, 1:4]
    #calculate weight for each class
    class_weights = 1 - tf.reduce_mean(cls_labels, axis=0)
    # your class weights
    #class_weights = tf.constant([[3.0, 1.0]])
    # deduce weights for batch samples based on their true label
    sample_weights = tf.reduce_sum(class_weights * cls_labels, axis=3)
    # sample_weights = tf.multiply(class_weights , cls_labels)
    # compute your (unweighted) softmax cross entropy loss
    unweighted_losses = tf.nn.softmax_cross_entropy_with_logits(
        labels = cls_labels, logits= cls_logits)
    # weighted_losses = tf.losses.sigmoid_cross_entropy(
    #     multi_class_labels=cls_labels, logits= cls_logits, weights=
    #     sample_weights )
    ...
    cls_loss = tf.reduce_mean(weighted_losses)
    cls_loss *= cfg.lambda_cls_loss
    ...
    class1_weights = tf.cast(tf.equal(y_true[:, :, :, 1], 1), tf.float32)
    class2_weights = tf.cast(tf.equal(y_true[:, :, :, 2], 1), tf.float32)
    class3_weights = tf.cast(tf.equal(y_true[:, :, :, 3], 1), tf.float32)
    #positive_weights = tf.cast(tf.equal(y_true[:, :, :, 0], 1), tf.float32)
    side_vertex_code_loss = \
        tf.reduce_sum(tf.reduce_sum(pos + neg, axis=-1) * class1_weights) / (
            tf.reduce_sum(class1_weights) + cfg.epsilon)
    side_vertex_code_loss += \
        tf.reduce_sum(tf.reduce_sum(pos + neg, axis=-1) * class2_weights) / (
            tf.reduce_sum(class2_weights) + cfg.epsilon)
    side_vertex_code_loss += \
        tf.reduce_sum(tf.reduce_sum(pos + neg, axis=-1) * class3_weights) / (
            tf.reduce_sum(class2_weights) + cfg.epsilon)
    # side_vertex_code_loss = \
    ...
    return inside_score_loss + cls_loss + side_vertex_code_loss + \
